...
/***/ function(module, exports, __webpack_require__) {
	...
	$("#enter").tap(function(){
	    ...
	    console.log(response.length);
	        ...
	        for(var i=0;i<response.length;i++){
	            $("#scroller ul").append("<li><span class='cg'>"+response[i].category+"</span><span>"+
	                response[i].name+"</span><span>项目名称："+
	                response[i].time+"</span><span>网址："+
	                response[i].level+"</span><span>项目描述："+
	                response[i].time+"</span><span>负责模块："+
	                response[i].time+"</span><span>运用技术："+
	                response[i].time+"</span></li>")
	        }
